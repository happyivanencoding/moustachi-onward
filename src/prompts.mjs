...
export function founderAnalyticsRequest(input={}){
 ...
 const retentionIntent=/(回访|回来|又回来|再次回来|再次访问|再次活跃|老用户|之前注册|此前注册|先前注册|return(?:ing|ed)?\s+users?|came\s+back|return\s+visit|retention|留存|reven(?:ir|u|us|ue|ues)|retour(?:ner|nés?|nées?)?|anciens?\s+utilisateurs?)/i.test(text);
 if(retentionIntent)return {mode:'retention',targets};
 ...
 const summaryIntent=/(tester|test\s*users?|测试(?:者|用户|码)|测试.{0,6}(?:人|人数)|测试码|邀请码|invite\s*codes?|registration|registered|注册|active\s*users?|活跃|funnel|漏斗|progress|进度|usage|使用情况|多少人|几个人|用了.*码|testeurs?|inscriptions?|utilisateurs?\s*actifs?|entonnoir|progression|utilisation)/i.test(text);
...
export function sanitizeFounderAnalyticsAggregate(value={}){
 ...
 const mode=['summary','retention','behavior','identity'].includes(String(value.mode||''))?String(value.mode):'summary';
 const result={
  available:true,mode,
 ...
 if(mode==='retention'&&value.retention&&typeof value.retention==='object'){
  const retentionUsers=(Array.isArray(value.retention.users)?value.retention.users:[]).slice(0,100).map(row=>({
   testerRef:/^tester-[a-f0-9]{10}$/.test(String(row?.testerRef||''))?String(row.testerRef):'',
   inviteCode:/^ONWARD(?:V1|\d{3,4})$/.test(String(row?.inviteCode||''))?String(row.inviteCode):'',
   authMode:['password','google'].includes(String(row?.authMode||''))?String(row.authMode):'',
   registeredAt:clean(row?.registeredAt,80),lastActiveBeforeToday:clean(row?.lastActiveBeforeToday,80),
   previousActiveDays:(Array.isArray(row?.previousActiveDays)?row.previousActiveDays:[]).slice(-8).map(day=>clean(day,20)).filter(Boolean),
   todayVisibleDurationMs:integer(row?.todayVisibleDurationMs),todayClickTotal:integer(row?.todayClickTotal),todaySessionCount:integer(row?.todaySessionCount),
  })).filter(row=>row.testerRef&&row.inviteCode);
  result.retention={
   day:clean(value.retention.day,20),returningUsersToday:integer(value.retention.returningUsersToday),
   inviteCodes:[...new Set((Array.isArray(value.retention.inviteCodes)?value.retention.inviteCodes:[]).map(code=>clean(code,64)).filter(code=>/^ONWARD(?:V1|\d{3,4})$/.test(code)))].slice(0,100),
   users:retentionUsers,
  };
 }

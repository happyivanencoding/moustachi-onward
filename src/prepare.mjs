import {buildOnwardFounderPrompt,buildOnwardOpsPrompt,founderAnalyticsRequest} from './prompts.mjs';
...
export function prepareInput(envelope,fetchAnalytics=fetchFounderAnalyticsAggregate){
  ...
  const analyticsRequest=founderAnalyticsRequest(data);
  if(analyticsRequest){try{data.analyticsAggregate=fetchAnalytics(analyticsRequest);}catch{data.analyticsAggregate={available:false};}}
  const instruction=input.origin==='whatsapp'?buildOnwardFounderPrompt(data):`You are the Onward profile of Moustachi, addressed by the authorized owner through the Moustachi API. Follow the current owner request and project authority C:\\dev\\onward\\DEEP_CONTEXT_HANDOFF_FINAL.md. The WhatsApp-only source-edit/new-VPS restrictions do not apply to this owner API channel; execute only what the current request actually authorizes. Use supplied fresh analytics for operational counts. Old discussion is context, not renewed authorization. Reply in Chinese.\nOWNER_REQUEST_AND_CONTEXT:\n${JSON.stringify(data)}\nHISTORY_EVIDENCE:\n${JSON.stringify(envelope.history||[])}`;
